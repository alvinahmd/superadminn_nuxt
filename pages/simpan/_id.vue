            ...
            <div class="pt-4">
              <a href="/pengguna">
                <div class="flex gap-2 hover:bg-blue-100 py-2 hover:rounded-md transition-all">
                  <img src="pengguna11.svg" alt="" class="pl-2">
                  <p style="color:  rgba(118, 118, 118, 1)">
                    Pengguna
                  </p>
                </div>
          ...
          <div class="bg-white flex flex-col-reverse md:flex-row w-full shadow-lg">
            <div class="flex md:flex-grow flex-col py-6 px-6">
              ...
              <div class="flex justify-end">
                <button class="uppercase hover:opacity-90 w-22 font-semibold bg-blue-700 text-white rounded-md focus:outline-none px-4 py-2" @click="simpan()">
                  Simpan
                </button>
              </div>
...
export default {
  ...
  methods: {
    ...
    async getData () {
      try {
        await this.$axios.$get('https://7fcf-2001-448a-5040-6858-95a1-522a-aa10-7a83.ngrok-free.app/api/users/' + this.$route.params.id, {
          headers: { 'ngrok-skip-browser-warning': '123123' }
        })
          .then((res) => {
            this.inputnamadep = res.message.first_name
            this.inputnamabel = res.message.last_name
            this.inputid = res.message.id_user
            this.inputjabatan = res.message.job
            this.inputpass = res.message.position
            this.inputgender = res.message.gender
            this.inputemail = res.message.email
            this.isactive = res.message.is_active
            this.inputpw = res.message.password
    ...
    async simpan () {
      try {
        await this.$axios.$put('https://7fcf-2001-448a-5040-6858-95a1-522a-aa10-7a83.ngrok-free.app/api/users' + this.$route.params.id, {
        ...
        },
        {
          ...
          this.$toast.success('Edit Akun Berhasil')
